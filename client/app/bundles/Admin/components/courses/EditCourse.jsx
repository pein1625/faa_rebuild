    ...
    axios.get(`/v1/courses/${id}/edit.json`)
      .then(response => {
        const {
          ...
          content, registration_deadline, cost, place, status = 'opening', course_category_id
        } = response.data.content.course;
        ...
        this.setState({
          name, description, start_date, end_date,
          content, registration_deadline, cost, place, status,
          course_category_id, categories, statuses, urls
        });
      })
      .catch(error => {
        console.log(error);
