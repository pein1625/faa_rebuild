...
class EditNews extends React.Component {
  ...
  componentDidMount() {
    let id = this.props.match.params.id;
    axios.get(`/v1/newses/new.json`)
      .then(response => {
        const {all_tags, categories} = response.data.content;
        const news_category_id = categories[0].id;
        this.setState({all_tags, categories, news_category_id});
      })
      .catch(error => {
        console.log(error);
      });
